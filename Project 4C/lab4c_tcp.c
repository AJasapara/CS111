...
void* servInput (void* pid) {
	...
	int count = 0;
	while (1) {
		bytesRead = read(socketfd, buf+offset, 32);
...
int main(int argc, char **argv) {
		...
		switch(opt){
			...
			case 's':
				if (strlen(optarg) == 1 && (toupper(optarg[0]) == 'C' || toupper(optarg[0]) == 'F'))
					...
					fprintf(stderr, "ERROR: Unrecognized argument.\nCorrect Usage: ./lab4c_tcp portnum --id=# --host=name --log=filename [--period=#] [--scale=[CF]]\n");
					exit(1);
				}
				break;
			case 'l': 
	...
	int c = connect(socketfd,(struct sockaddr *) &addr, sizeof(addr));
	if(c < 0) { 
		fprintf(stderr, "ERROR: %s\n", strerror(errno));  
		exit(2); 
	}
